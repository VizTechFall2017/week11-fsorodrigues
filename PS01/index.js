...
var albersProjection = d3.geoAlbersUsa()            //tell it which projection to use
                          .scale((width))     //tell it how big the map should be
...
svg.call(texture); // calling pattern in the svg context

// setting up file loading queue
queue()
  .defer(d3.json, "./data/cb_2016_us_state_20m.json")  // import the map geoJSON
  .defer(d3.csv, "./data/beachnourishment_2016.csv")   // import the data from the .csv file
  .defer(d3.csv, "./data/state_totals.csv")            // import the data from the .csv file
  .defer(d3.json, "./data/cities_us.json")             // import the data from the .json file
  .await(function(err, dataIn, circleData, stateData, usCities) {
    ...
    var centroids = dataIn.features.map(function (feature){
            return {state: feature.properties.NAME, stateID: feature.properties.STUSPS, center: path.centroid(feature)};
    });
    ...
    svg.append("g")
        ...
        .attr("id", function(d) { return d.stateID; })
...
function showCountry(speed) {
    ...
    d3.selectAll(".legend").transition().duration(speed).attr("opacity", 0);
...
function showProjects(speed) {
  ...
  d3.selectAll(".circles")
    ...
    d3.selectAll(".legend-country").transition().duration(speed).attr("opacity", 0);
...
function mouseoverProjects(d,i) {
      ...
      tooltip.transition()
       ...
       tooltip.style("left", function(d) { if (cx > 600) {
                                             return (cx - 250) + "px";
                                         } else {
                                             return (cx + 20) + "px"
              ...
              .style("top", function() { return (cy) + "px" })
...
function mouseoverStates(d,i) {
        ...
        var cy = +thisEl.attr("cy");
            ...
            stateTooltip.style("opacity", 1)
                        .style("left", function(d) { if (cx > 600) {
                                                        return (cx - 120) + "px"
                                                   } else {
                                                        return (cx + 20) + "px"
                                                   }
